...
class bookNav extends plugin 
{
  ...
  function mkLink($url,$label)
  {
    ...
    switch($linkTargetType)
    {
      ...
      else
      {
        ...
        if(strstr($url, $thisDir) && 
          preg_replace(':/$:','',$_SESSION['currentDirUrl']) != $_SESSION['bookTop'])
         $link = '<a class="highlighted" href="?robopage='.$url.'">' 
          . $label . '</a>' . "\n";
        else
  ...
  function assembleLocalPageLinks($linksString)
  {
    ...
    for($i=0; $i<$cnt; $i++)
    {
      ...
      //$label = StaticRoboUtils::mkLabel($label);
...
ENDO;
  ...
  function getOutput($divid)
  {
    ...
    $mess = '<p style="margin-left: 1em;">' . $mess . '</p>';
    $this->getGlobalChapterLinks();
    $cnt = count($this->globalChapterLinks);
    for($i=0; $i<$cnt; $i++)
    {  
       $top .= $this->globalChapterLinks[$i];
        $dom->loadHTML($this->globalChapterLinks[$i]);
        $nodes = $dom->getElementsByTagName('a');
        $href = str_replace("?robopage=","",$nodes[0]->getAttribute("href"));
       if($href == preg_replace(":/$:","",$_SESSION['currentDirUrl']))
       {
           $top .= $mess;
       }
    }
